...
def messages_handler(message):
    ...
    if (re.match('(^|\w*)да$', message.text.lower())):
        bot.send_message(message.chat.id, 'пизда')
        return

    if (re.match('(^|\w*)нет$', message.text.lower())):
        bot.send_message(message.chat.id, 'пидора ответ')
        return

    otvetki = ['баланду не малявим фраерок', 'мусор по тебе плачет аквалангист-кун', 'кто много ругается в того ракеты летят (с. Батюшка)', 
